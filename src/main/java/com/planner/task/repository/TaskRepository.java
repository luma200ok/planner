...
public interface TaskRepository extends JpaRepository<Task, Long> {
    ...
    @Query("""
            select new com.planner.report.application.dto.DailyReportRow(
    ...
    @Query("""
            select new com.planner.report.application.dto.SummaryRow(
            count(t),
            sum(case when t.status = com.planner.task.domain.TaskStatus.DONE then 1 else 0 end),
            sum(case when t.status = com.planner.task.domain.TaskStatus.SKIPPED then 1 else 0 end ),
            sum(case when t.status = com.planner.task.domain.TaskStatus.PLANNED then 1 else 0 end))
    ...
    @Query("""
            select new com.planner.report.application.dto.TemplateRow( +
                        tt.id,
                        tt.title,
    ...
    List<TemplateRow> templateRow(LocalDate from, LocalDate to);
    ...
    @Query("""
            select new com.planner.report.application.dto.TemplateRow(
             ...
             from Task t
             where t.template is null and t.scheduledDate between :from and :to
            """)
    TemplateRow oneOffRow(
            @Param("templateId")Long templateId,@Param("from") LocalDate from, @Param("to") LocalDate to);
}
